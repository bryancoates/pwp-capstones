...
# Set variables for intro letters
...
def get_average_sentence_length(text):
    """Determine average length of sentences in given text"""

    # Replace sentence endings so we can easily define a sentence
    ...
    # Determine and return the average words per sentence
...
def prepare_text(text):
    """Replace all punctuation and return a list of lowercase words"""

    # Create new variable with lower text
    new_text = text.lower()
    
    # Define a list of special characters
    special_characters = [",", "'", ".", "!", "?"]
    
    # Loop through the speical characters and replace
...
def build_frequency_table(corpus):
    """Determine the frequency of words in a provided list"""

    # Create an empty dictionary to store values
    frequency_table = {}
    
    # Count each word and add to dictionary
    for check_word in corpus:
        # Only count words not counted already
        if check_word not in frequency_table:
            i = 0
            for word in corpus:
                if word == check_word:
                    i += 1
            # Add the word and count to dictionary
            frequency_table.update({check_word: i})
...
def ngram_creator(text_list):
    """Return a list of two-word n-grams for given text"""
    
    # Create empty list to store n-grams
    ngram_list = []
    
    # Loop through the text creating two-word n-grams
...
def frequency_comparison(table1, table2):
    """Compare two frequency tables and return a score"""
    
    # Define count variables
    appearances = 0
    mutual_appearances = 0
    
    # Compare table1 to table2 totalling values
    for key in table1:
        if key in table2:
...
def percent_difference(value1, value2):
    """Return percent difference between two values"""
...
def find_text_similarity(sample1, sample2):
    ...
    # Return the average of the 3 similarity scores
...
# Create variables to hold the muderer score and name
murderer = "Unknown"
murderer_score = 0

# Print out the author and similarity score
for sample in [lily_sample, myrtle_sample, gregg_sample]:
    author = sample.author
    similarity_score = find_text_similarity(sample, murderer_sample)
    print("{} has a similarity score of {} when compared to the murder letter".format(author, similarity_score))
    if similarity_score > murderer_score:
        murderer_score = similarity_score
        murderer = author
...
# Print the name of the killer
print(murderer + " killed Jay Stacksby!")
